...
contract MAIN{
    ...
    public
    {
      require(token.balanceOf(msg.sender) >= PRICE * 10**18,
       "Not Enough Balance To Mint NFT.");
      require(IERC721customs(openApe).addrExist(_to)!= true,
       "You can't mint Hammer as you own Open APE");
       try IERC721customs(hammer).mintNFT(_to,_tokenURI) returns(string memory result){
    ...
    public{
      require(token.balanceOf(msg.sender)>= PRICE * 10**18,
       "Not Enough Balance To Mint NFT.");
      require(IERC721customs(hammer).addrExist(_to)!= true,
       "You can't mint OpenApe as you own Open Hammer");
       try IERC721customs(openApe).mintNFT(_to,_tokenURI) returns(string memory result){
           token.transferFrom(_to, address(this), PRICE*10**18);
           emit Log(result);
       }catch{
           emit Log("External call failed");
